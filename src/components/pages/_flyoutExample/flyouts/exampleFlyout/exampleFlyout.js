...
export class ExampleFlyout extends Component {
  ...
  getSummaryMessage() {
    ...
    if (isPending) {
      return t('examples.flyout.flyouts.example.pending');
    } else if (changesApplied) {
      return t('examples.flyout.flyouts.example.applySuccess');
    } else {
      return t('examples.flyout.flyouts.example.affected');
    }
  }
  ...
  render() {
    ...
    return (
      <Flyout>
        <FlyoutHeader>
          <FlyoutTitle>{t('examples.flyout.flyouts.example.header')}</FlyoutTitle>
        ...
        <FlyoutContent>
          ...
          <form className="example-flyout-container" onSubmit={this.apply}>
            <div className="example-flyout-header">{t('examples.flyout.flyouts.example.header')}</div>
            <div className="example-flyout-descr">{t('examples.flyout.flyouts.example.description')}</div>

            <div className="form-placeholder">{t('examples.flyout.flyouts.example.insertFormHere')}</div>

            {/** Sumarizes the action being taken; including count of items affected & status/pending indicator */}
            <SummarySection>
              <SectionHeader>{t('examples.flyout.flyouts.example.summaryHeader')}</SectionHeader>
              <SummaryBody>
            ...
            {error && <AjaxError className="example-flyout-error" t={t} error={error} />}
            {
              ...
              !changesApplied &&
              <BtnToolbar>
                <Btn svg={svgs.reconfigure} primary={true} disabled={isPending || itemCount === 0 } type="submit">{t('examples.flyout.flyouts.example.apply')}</Btn>
                <Btn svg={svgs.cancelX} onClick={onClose}>{t('examples.flyout.flyouts.example.cancel')}</Btn>
              </BtnToolbar>
            }
            {
              /**
              ...
              !!changesApplied &&
              <BtnToolbar>
                <Btn svg={svgs.cancelX} onClick={onClose}>{t('examples.flyout.flyouts.example.close')}</Btn>
              </BtnToolbar>
            }
          </form>
        </FlyoutContent>
